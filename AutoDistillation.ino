...
#define VERSION "231117.1"  // Версия ПО. ДеньМесяцГод.номер
...
#define SENSOR_0   0.70     // Корректировка Т куба
#define SENSOR_1   0.70     // Корректировка Т дефл,                                  по воде была поправка -0.10, но по спирту нужно +0.20 добавить, ДЕЛАЮ ПОПРАВКУ ДЛЯ ОБЕИХ ДАТЧИКОВ, Т.К. НА БУДУЩЕЕ ПЛАНИРУЮ ПО Т КУБА УЗНАВАТЬ КРЕПОСТЬ НАВАЛКИ
...
#define BARDA_MIN_DLT 0.2   // Минимальная дельта для барды, потери 1.3% л АС.
#define BARDA_MAX_DLT 0.4   // Максимальная дельта для барды, потери 2.5% л АС.
...
byte CubeThermometer[8] = {0x28, 0xFF, 0xF4, 0x63, 0xA4, 0x15, 0x04, 0xF3}; //RED - КРАСНЫЙ
byte DeflThermometer[8] = {0x28, 0xFF, 0x22, 0x53, 0xA4, 0x15, 0x04, 0x79};  //BLUE - СИНИЙ
...
const char * Levels[] = {"STOP", "RAZGON", "NA_SEBYA", "GOLOVY", "TELO", "HVOSTY", "DISPLAY", "BRG NBK", "CUBE", "PRDTELO1", "PRDTELO2", "DELAY"}; // Уровни задач дистилляции
...
const char * DistillTasks[] = {"DIST 92/4", "RECT_1/93/4", "DIST 92/5", "RECT_2/93/4", "CUBE"};  // Задачи дистилляции/ректификации цифра Т куба для завершения отбора тела
